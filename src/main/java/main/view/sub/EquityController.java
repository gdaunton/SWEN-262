        ...
        apply.setOnAction(new EventHandler<ActionEvent>() {
            public void handle(ActionEvent event) {
                ...
                if(!outside.isSelected()) {
                    ...
                    if (transaction < 0)
                        controller.sendCommand(HoldingCommand.Action.MODIFY, accounts.getSelectionModel().getSelectedItem(), HoldingCommand.Modification.WITHDRAW, Math.abs(transaction));
                    else
                        controller.sendCommand(HoldingCommand.Action.MODIFY, accounts.getSelectionModel().getSelectedItem(), HoldingCommand.Modification.DEPOSIT, transaction);
                }
